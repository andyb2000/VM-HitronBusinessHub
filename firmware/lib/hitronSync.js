...
Backbone.sync = function(method, model, options) {
    ...
    if (Backbone.emulateJSON) {
      ...
      params.data = params.data ? {model: params.data, csrf: $("#csrf_token").val()} : {};
    }
    ...
    //reset session time
    if (params.url.indexOf("/1/Device/Users/Alive") == -1) {
        SessionResetFlag = 1;
    }
    // Make the request, allowing the user to override any Ajax options.
    return Backbone.ajax(_.extend(params, options)).done(function(data){
      if(data){
        if (data.errMsg == "redirect" && data.redirectUrl){
          if (data.redirectUrl.substr(0, 4) != "http")
            data.redirectUrl = "http://" + data.redirectUrl;
